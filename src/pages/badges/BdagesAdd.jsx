...
export default function BdagesAdd({ getBadges, setisBadgeOpen, id }) {
    ...
    const getApplicable = async () => {
        const res = await apiRequest("GET", API_ROUTES.common.getBadgeApplicableFor, null, null, {
            showLoader: true
        })
    ...
    const badgesSave = async (event) => {
        ...
        if (id) {
            badgesValue.id = id
            res = await apiRequest("PUT", API_ROUTES.badges.badgeUpdate, badgesValue, null, {
                showLoader: true,
                showToaster: true
            })
        }
            ...
            res = await apiRequest("POST", API_ROUTES.badges.badgesAdd, badgesValue, null, {
                showLoader: true,
                showToaster: true
            })
        }
    ...
    const getByBadgeId = async () => {
        const res = await apiRequest("GET", API_ROUTES.badges.badgeById(id), null, null, {
            showLoader: true
        })
                    ...
                    <form onSubmit={badgesSave} action="">
                        <div className="modal-body px-4 py-3">
                            ...
                            <div className="mb-3">
                                ...
                                >
                                    {
                                        applicableData?.map((value, index) => <option
                                            key={value?.id}
