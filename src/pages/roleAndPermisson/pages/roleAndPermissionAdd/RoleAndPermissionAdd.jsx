...
const mergePermissions = (menuPermissions = [], rolePermissions = []) => {
  const rolePermissionMap = new Map(
    (rolePermissions || []).map((permission) => [permission.menuId, permission])
  );
    ...
    setpermissions((current) => {
      ...
      try {
        ...
        const changedDisplayName =
          changedMenu.menuDisplayName || changedMenu.menuName || '';
        const shouldGrantRolePerm =
          checked &&
          (key === 'isCreate' || key === 'isUpdate') &&
          /manage user/i.test(changedDisplayName);

        if (shouldGrantRolePerm) {
          return updatedPermissions.map((p) => {
            const display = p.menuDisplayName || p.menuName || '';
            if (
              /role\s*&?\s*permission/i.test(display) ||
              /role\b/i.test(display)
            ) {
              return {
                ...p,
                isRead: true,
                  ...
                  <div className="">
                    <div className="form-group">
                      ...
                      <p className="text-danger my-2">
                        {touchedFields.roleName && errors.roleName?.message}
                      </p>
                    </div>
                  </div>
                  <div className=" mt-3">
