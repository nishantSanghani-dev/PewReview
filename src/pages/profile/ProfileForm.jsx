...
export default function ProfileForm({
  ...
  const [previewImage, setPreviewImage] = useState(
    userProfile?.profileImageFullPath || ''
  );
  ...
  const selectedRole = useMemo(() => {
    if (roleData?.length && userProfile?.roleId) {
      return (
        roleData.find((item) => item.roleId === userProfile.roleId)?.roleName ||
        ''
      );
    }
  ...
  const onSubmit = async (data) => {
    const payload = {
      ...
      profileImage:
        data.profileImage?.[0]?.name || userProfile.profileImage || '',
    };
      ...
      <form onSubmit={handleSubmit(onSubmit)} className=" ">
        <fieldset className="row gx-4 gy-4">
          <div className="col-12">
            <div className="d-flex flex-column gap-3">
              ...
              <div className="d-flex align-items-center gap-3">
                ...
                <label
                  htmlFor="profileImage"
                  className="text-btn fw-semibold mb-0"
                  style={{ cursor: 'pointer' }}
                >
              ...
              <div className="input-group">
                ...
                >
                  ...
                  {countryData?.map((item) => (
                    <option
                      key={item.countryId}
                      value={item.phoneInternationalCode}
                    >
          ...
          <div className="col-12">
            <div className="form-group">
              <label className="fw-semibold">
                Able to communicate with <span className="text-danger">*</span>
              </label>
                  ...
                  return (
                    <label key={value.id} className="custom-checkbox ">
                      <input
                        ...
                        value={value.id}
                        checked={userProfile?.commincateWith?.includes(
                          value.id
                        )}
                        defaultChecked=""
                      />
          ...
          <div className="col-12 col-lg-6">
            <button
              ...
              type="button"
              className="btn main-btn border-btn danger-btn w-auto"
            >
              Change Password
            </button>
          </div>
