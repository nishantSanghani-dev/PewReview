...
export default function VenueAdd({
  ...
  const getEndUuser = async () => {
    const res = await apiRequest(
      'GET',
      API_ROUTES.endUser.endUserDropDown,
      null,
      null,
      {
        showLoader: true,
  ...
  const getVenue = async () => {
    const res = await apiRequest(
      'GET',
      API_ROUTES.common.getVenueType,
      null,
      null,
      {
        showLoader: true,
  ...
  const findMatchingCountry = (data) => {
    const addressText =
      `${data?.address || ''} ${data?.countryCodeName || ''} ${data?.countryCode || ''}`.toLowerCase();
  ...
  const initializeMap = () => {
    ...
    let marker = window.L.marker([defaultLat, defaultLng], {
      draggable: true,
    }).addTo(map);
  ...
  const handleCountry = (event) => {
    ...
    const contryFind = countryData.find(
      (value, index) => value.countryId == event.target.value
    );
  ...
  const handleSave = async (data) => {
    ...
    const selectedCountry = countryData.find(
      (c) => String(c.countryId) === String(data.selectedCountryId)
    );
    const countryCode =
      data.countryCode ||
      (selectedCountry ? selectedCountry.phoneInternationalCode : '');
    ...
    const payload = {
      ...
      phone: data.phone,
      userId:
        selectedUser?.key || selectedUser?.userId || selectedUser?.id || '',
    ...
    const url = editVenueId
      ? API_ROUTES.venue.venueEdit(editVenueId)
      : API_ROUTES.venue.VenueAdd;
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        ...
        !venueDropdownRef.current.contains(event.target)
      ) {
        setisVenuOpen(false);
      }
    };
  ...
  useEffect(() => {
    const processSingleVenue = () => {
      ...
      setcountryDetails({
        ...
        if (matchingCountry) {
          setValue('selectedCountryId', matchingCountry.countryId, {
            shouldValidate: true,
          });
          setValue(
            'countryCode',
            matchingCountry.phoneInternationalCode || '',
            { shouldValidate: true }
          );
          setValue('countryCodeName', matchingCountry.countryCode || '', {
            shouldValidate: true,
          });
        } else {
          ...
          const venueCountryCode =
            ...
            '';
          const venueCountryCodeName =
            data?.countryCodeName || countryDetails?.countryCodeName || '';
          ...
          const fallbackCountry =
            countryData.find((c) => {
              ...
              return (
                (venueCountryCode &&
                  phone.includes(
                    String(venueCountryCode).replace(/\+/g, '')
                  )) ||
          ...
          if (fallbackCountry) {
            setValue('selectedCountryId', fallbackCountry.countryId, {
              shouldValidate: true,
            });
            setValue(
              'countryCode',
              fallbackCountry.phoneInternationalCode || '',
              { shouldValidate: true }
            );
            setValue('countryCodeName', fallbackCountry.countryCode || '', {
              shouldValidate: true,
            });
          }
        }

        const matchingVenueType = getVenueType.find(
          (item) =>
