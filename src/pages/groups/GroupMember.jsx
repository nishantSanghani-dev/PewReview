    ...
    <td {...props.tdProps}>
      <div className="d-flex gap-2 align-items-center">
        <img
          ...
          src={props?.dataItem?.profileImagePath}
          alt=""
        />
        {}
        {endUserPermission?.canRead ? (
          <Link
            ...
            to={`/admin/manage-end-user/view/${props.dataItem.userId}`}
          >
            {props?.dataItem?.userName}
          </Link>
        ) : (
          <span className="">{props?.dataItem?.userName}</span>
        )}
...
export default function GroupMember() {
  ...
  const getMemeberList = async () => {
    const res = await apiRequest(
      'POST',
      API_ROUTES.groups.getGroupMemberList,
      { pageNumber: 1, pageSize: 10, groupId: id },
      null,
      {
        showLoader: true,
      }
  ...
  return (
    <div className="container-fluid  ">
      ...
      <BreadCumb
        items={[
          { label: 'Group Details', path: `/admin/groups/view/${id}` },
          { label: 'Group Members' },
        ]}
      />
