...
export default function AmmunitionAdd({
  id,
  setid,
  isAmmunitionOpen,
  setisAmmunitionOpen,
  ...
  const getSingleRecord = async () => {
    const res = await apiRequest(
      'GET',
      API_ROUTES.ammunition.ammunitionGetById,
      null,
      { id },
      {
        showLoader: true,
    ...
    if (res.status && res.data) {
      const fetchedCategoryIds =
        ...
        (res.data.categoryId ? [res.data.categoryId] : []);
  ...
  const onSubmit = async (data) => {
    ...
    if (id) {
      ...
      res = await apiRequest(
        'PUT',
        API_ROUTES.ammunition.ammunitionUpdate,
        data,
        null,
        {
          showLoader: true,
  ...
  const handleSelectCategory = (item) => {
    ...
    const isSelected = selectedCategories.some(
      (c) => String(c.categoryId || c.id || c.value) === itemId
    );
    let updated = [];
    if (isSelected) {
  ...
  const handleRemoveCategory = (item) => {
    ...
    setSelectedCategories(updated);
    const updatedIds = updated.map((c) =>
      String(c.categoryId || c.id || c.value)
    );
    setValue('categoryIds', updatedIds, { shouldValidate: true });
    trigger('categoryIds');
  };
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="modal-body px-4 py-3">
              ...
              <div className="form-group mb-3">
                ...
                <div
                  className={`custom-dropdown ${errors.categoryIds ? 'is-invalid' : ''}`}
                ...
                >
                  ...
                  >
                      ...
                      {selectedCategories.length === 0 ? (
                        ...
                        <>
                          ...
                          {selectedCategories.length > 1 && (
                            ...
                            >
                              <span>
                                {selectedCategories.length - 1} item selected
                              </span>
                              <span
                                onClick={(e) => {
                                  e.stopPropagation();
                    ...
                    >
                      ...
                      <i
                        className={`demo-icon ${isCategoryDropdownOpen ? 'icon-angle-up' : 'icon-angle-down'}`}
                      ></i>
                    </div>
                  </div>
                  ...
                  {isCategoryDropdownOpen && (
                    ...
                    >
                      {categories.map((item, index) => {
                        ...
                        const isSelected = selectedCategories.some(
                          (c) =>
                            String(c.categoryId || c.id || c.value) === itemId
                        );
                        return (
                          <div
                            key={index}
              ...
              <div className="form-group mb-3">
                ...
                >
                  ...
                  {manufacturers.map((mfg) => {
                    const mfgId =
                      mfg.key || mfg.id || mfg.manufacturerId || mfg.value;
                    const mfgName =
                      mfg.name || mfg.manufacturerName || mfg.text || mfg.value;
                    return (
