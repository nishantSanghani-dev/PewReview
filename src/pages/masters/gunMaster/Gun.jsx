...
export default function Gun({ isAddGunOpen, setIsAddGunOpen, getGun, id, setId }) {
    ...
    const getCategories = async () => {
        const res = await apiRequest("GET", API_ROUTES.category.categoryDropdown, null, { applicableFor: 1 }, { showLoader: true });
        if (res?.data) setCategoryData(res.data);
    };

    const getManufacturers = async () => {
        const res = await apiRequest("GET", API_ROUTES.manufacturer.manufacturerDropdown, null, null, { showLoader: true });
        if (res?.data) setManufacturerData(res.data);
    };

    const getAmmunition = async () => {
        const res = await apiRequest("GET", API_ROUTES.ammunition.ammunitionDropDown, null, null, { showLoader: true });
        // The API returns {key, value} according to the prompt
        if (res?.data) setAmmunitionData(res.data);
    };

    const getSingleRecord = async () => {
        const res = await apiRequest("GET", API_ROUTES.gun.gunGetById, null, { id }, {
            showLoader: true
        });
        if (res?.data) {
            const data = res.data;
            reset({
                ...
                imageNames: data.attachments && data.attachments.length > 0 ? data.attachments[0].attachmentName : null
            });
    ...
    const renderMultiSelect = (itemsData, selectedIds, isOpen, setIsOpen, toggleFunc, setValueField, placeholder) => {
        const selectedItems = itemsData.filter(item => selectedIds.includes(item.key));
        ...
        return (
            ...
            >
                <div
                    ...
                    style={{ height: 'auto', minHeight: '40px', padding: '6px 14px', borderRadius: '10px' }}
                    onClick={() => setIsOpen(!isOpen)}
                >
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', width: '90%' }}>
                        {selectedItems.length === 0 ? (
                            <span style={{ color: '#6c757d', paddingTop: '2px' }}>{placeholder}</span>
                        ) : (
                            <>
                                <div style={{
                                    display: 'inline-flex',
                                ...
                                {selectedItems.length > 1 && (
                                    <div style={{
                                        ...
                                        <span
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setValue(setValueField, [selectedItems[0].key], { shouldValidate: true });
                                            }}
                                            style={{
                                                marginLeft: '8px',
                                                cursor: 'pointer',
    ...
    return (
        <div className="modal fade show d-block" tabIndex="-1" style={{ backgroundColor: "rgba(0,0,0,0.55)" }}>
            <div className="modal-dialog modal-dialog-centered modal-xl">
                <div className="modal-content border-0" style={{ borderRadius: "14px", overflow: "hidden" }}>
                    {/* Header */}
                    <div className="modal-header border-bottom px-4 py-3">
                        <h3 className="modal-title fw-bold mb-0" style={{ fontSize: "20px" }}>{id ? "Edit Gun" : "Add Gun"}</h3>
                        <button type="button" className="btn-close fs-5" onClick={handleClose}></button>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(handleSave)}>
                        <div className="modal-body px-4 py-3">
                            <div className="row">
                                ...
                                <div className="col-12 col-lg-6 mb-3">
                                    ...
                                    />
                                    {errors.gunName && (
                                        <div className="invalid-feedback d-block">{errors.gunName.message}</div>
                                    )}
                                </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-12 col-lg-6 mb-3">
                                    ...
                                    {errors.manufacturerIds && (
                                        <div className="invalid-feedback d-block">{errors.manufacturerIds.message}</div>
                                    )}
                                </div>

                                {/* Barrel Length */}
                                <div className="col-12 col-lg-6 mb-3">
                                    <label className="fw-semibold form-label">Barrel Length</label>
                                    <input
                                        type="text"
                                    ...
                                    />
                                    {errors.barrelLength && (
                                        <div className="invalid-feedback d-block">{errors.barrelLength.message}</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="row">
                                ...
                                <div className="col-12 col-lg-6 mb-3">
                                    ...
                                    {errors.ammunitionIds && (
                                        <div className="invalid-feedback d-block">{errors.ammunitionIds.message}</div>
                                    )}
                                </div>

                                {/* Approval Status */}
                                <div className="col-12 col-lg-6 mb-3">
                                    <label className="fw-semibold form-label">Approval Status</label>
                                    <select
                                        {...register("approvalStatus", { valueAsNumber: true })}
                                        className="form-select"
                                        style={{ height: "40px", borderRadius: "10px", backgroundColor: "#e9ecef" }}
                                        disabled
                                    >
                            ...
                            <div className="row">
                                ...
                                <div className="col-12 col-lg-6 mb-3">
                                    ...
                                    />
                                    {errors.details && (
                                        <div className="invalid-feedback d-block">{errors.details.message}</div>
                                    )}
                                </div>
                                ...
                                <div className="col-12 col-lg-6 mb-3">
                                    ...
                                    <label
                                        ...
                                        style={{ cursor: 'pointer', overflow: 'hidden', height: '40px', border: '1px solid #dee2e6', borderRadius: '10px' }}
                                    >
                                        <div style={{
                                    ...
                                    {errors.imageNames && (
                                        <div className="invalid-feedback d-block">{errors.imageNames.message}</div>
                                    )}
                                        ...
                                        <div style={{ marginTop: '12px', position: 'relative', display: 'inline-block' }}>
                                            ...
                                            <button
                                                ...
                                                onClick={(e) => {
                                                    ...
                                                    setValue("imageNames", null, { shouldValidate: true });
                        ...
                        <div className="modal-footer px-4 py-3">
                            <button
                                ...
                                style={{ borderRadius: "10px", minWidth: "100px", height: "40px" }}
                                onClick={handleClose}
                            >
                                Cancel
                            </button>
